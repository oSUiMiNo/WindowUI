...
public class UwcWindowTexture : MonoBehaviour
{
    ...
    public bool isValid
    {
        get { return window != null && window.isValid; }
    }
    ...
    public Subject<Unit> On_ScaleChanged = new Subject<Unit>(); //加筆大平
    public static Subject<Unit> On_IncOrDec = new Subject<Unit>(); //加筆大平
    ...
    void Awake()
    {
        ...
        list_.Add(this);
        On_IncOrDec.OnNext(Unit.Default); //加筆大平
        }
    ...
    void UpdateScale()
    {
        ...
        if(transform.lossyScale == scale) return; // 加筆大平
        transform.localScale = scale;
        On_ScaleChanged.OnNext(Unit.Default); // 加筆大平
    }
